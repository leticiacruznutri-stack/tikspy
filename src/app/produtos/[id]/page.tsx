...
import { getProductById, getAnalysis, saveAnalysis } from "@/lib/store";
    ...
    <div className="space-y-4">
      ...
      {analysis && (
        <>
          ...
          {/* Hooks de referência */}
          {analysis.referenceHooks && analysis.referenceHooks.length > 0 && (
            <div className="rounded-2xl bg-white border border-[#e8e0d4] p-4 space-y-3">
              <h2 className="text-sm font-semibold text-[#1a1a2e] flex items-center gap-2">
                <Zap size={14} className="text-[#f59e0b]" />
                Hooks de referência
                <span className="text-[11px] font-normal text-[#9ca3af]">({analysis.referenceHooks.length})</span>
              </h2>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {analysis.referenceHooks.map((hook, i) => (
                  <div key={i} className="rounded-xl border border-[#f0ebe3] px-3 py-2.5 group hover:border-[#c8b99a] transition-colors">
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-[13px] font-medium text-[#1a1a2e] leading-snug flex-1">
                        &ldquo;{hook.headline}&rdquo;
                      </p>
                      <button
                        onClick={async () => {
                          const updated = analysis.referenceHooks!.filter((_, j) => j !== i);
                          await saveAnalysis({ ...analysis, referenceHooks: updated });
                          setAnalysis({ ...analysis, referenceHooks: updated });
                        }}
                        className="rounded p-1 text-[#e8e0d4] hover:text-[#fe2c55] opacity-0 group-hover:opacity-100 transition-all shrink-0"
                      >
                        <Trash2 size={12} />
                      </button>
                    </div>
                    <div className="flex items-start gap-1.5 mt-1.5">
                      <Eye size={10} className="text-[#b8a88a] mt-0.5 shrink-0" />
                      <p className="text-[11px] text-[#9ca3af] leading-snug">{hook.visualHook}</p>
                    </div>
                  </div>
                ))}
              </div>
